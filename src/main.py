...
#
# POS_WINDOW_SIZE = 15  # 위치 스무딩 윈도우 크기
# SPEED_WINDOW_SIZE = 15  # 속도 중앙값 필터 윈도우 크기
# COOLDOWN_FRAMES = 15  # 이상 움직임 감지 시 쿨다운 프레임
# ABNORMAL_THRESH = 7  # 점프 등 이상 움직임 감지 임계값 (픽셀)
# MIN_MOVEMENT_DIST = 0.05  # 히스토그램 집계를 위한 최소 이동 거리 (미터)
#
...
#         if directions:
#             # plot_direction_histogram 함수가 파일을 직접 저장한다고 가정
...
#
#     heatmap_output_dir = os.path.join(os.path.dirname(args.output_path), "heatmap")
...
#     if not cap.isOpened():
#         print(f"Error: Failed to open video file: {args.input_path}")
#         return
#
...
#         print("Error: Could not compute homography matrix. Exiting.")
#         return
#
#     # 미니코트 설정
#     mini_court = MiniCourt(first_frame)
#     mc_height_px = mini_court.convert_meters_to_pixels(TENNIS_COURT_HEIGHT_M)
#     real_court_dims = (TENNIS_COURT_WIDTH_M, TENNIS_COURT_HEIGHT_M)
...
#         'movement_directions': {},  # angle list
#         'minicourt_positions': {},  # visualization points
#         'kalman_states': {},
#         'real_pos_history': {},  # deque for smoothing
#         'smoothed_real_pos': {},
#         'speeds_history': {},  # deque
#         'last_y_center': {},
#         'cooldown_counter': {},
#         'stable_foot_pos': {}
#     }
...
#                         dy = -(curr_smooth_pos[1] - prev_smooth_pos[1])  # Y축 반전
#
...
#                             if median_speed < 11.0:  # 11m/s 이상은 오류 가능성 높음
...
#                 foot_pos_mc = transform_real_to_minicourt(foot_pos_real, real_court_dims, mc_dims, mc_origin)
...
#
#     # 히트맵 & 히스토그램 생성
#     generate_post_processing_outputs(stats, first_frame, mini_court, args.name, heatmap_output_dir)
...
#     default_name = 'result'
#
#     parser.add_argument('--input_path', type=str, default=default_input, help='Path to input video')
#     parser.add_argument('--tracker_model', type=str, default=default_tracker, help='Path to YOLO tracking model')
#     parser.add_argument('--court_model', type=str, default=default_court, help='Path to Court Keypoints model')
#     parser.add_argument('--name', type=str, default=default_name, help='Project name for output files')
#
